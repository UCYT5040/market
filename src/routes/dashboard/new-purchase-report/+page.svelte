...
<div class="flex h-full flex-col items-center justify-center">
	<div class="max-h-7/8 w-full max-w-lg overflow-y-auto card preset-outlined-surface-500 p-4">
		<h1 class="mb-6 text-center text-2xl font-bold">Purchase Report</h1>
		<h2 class="text-lg font-semibold">Minimum Requirements</h2>
		<ol>
			<li>Items should be no more expensive than 1.1x the local price</li>
			<li>
				Your sweat equity gain is determined by how much cheaper you can find the item compared to
				the local price
			</li>
			<li>You must provide a photo of the receipt or proof of purchase</li>
		</ol>
		<h2 class="text-lg font-semibold">Important Tax Information</h2>
		<p>
			Taxes must be accounted for within your purchase report. You are asked to enter an address of
			purchase for tax calculation. If the purchase was made online or there is another tax-related
			exemption, please explain in the notes section.
		</p>
		<p>
			<strong>For Total Price:</strong> Include taxes in the total price you enter.
		</p>
		<p>
			<strong>For Itemized Prices:</strong> Enter the price of each item without taxes. Taxes will be
			added to the total automatically.
		</p>
		<p>
			For more information on taxes, as well as a calculator, see <a href="/dashboard/taxes">
				Taxes
			</a>
		</p>
		<form class="space-y-4" method="POST">
			...
			{#if pricingType === 'total'}
				...
				<p class="mb-4 text-sm text-gray-500">This should include sales tax.</p>
			{/if}
			...
			<label class="label">
				...
				<select bind:value={taxIndex} class="select p-2" name="taxRateIndex">
			...
			{#if taxIndex}
				<p class="text-sm text-gray-500">
					Selected tax rate: {data.taxRates[parseInt(taxIndex)].percent ?? 'N/A'}
				</p>
				{#if pricingType === 'total'}
			...
			{/if}
			<p>
				You will be required to upload photo evidence of your purchase after submitting this report.
			</p>
			<button class="btn w-full preset-filled-primary-500" type="submit"> Submit Report </button>
		</form>
	</div>
</div>
